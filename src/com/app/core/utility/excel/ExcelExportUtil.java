...
import com.app.core.annotation.ExcelCollection;
...
public final class ExcelExportUtil {
			...
			if (entity.getList() != null) {
				...
				if (entity.getType() == 1) {
					createStringCell(row, cellNum++,
							...
							index % 2 == 0 ? getStyles(styles, false, entity.getAlign())
									: getStyles(styles, true, entity.getAlign()),entity);
				} else {
	...
	private static Object getCellValue(ExcelExportEntity entity, Object obj) throws Exception{
		...
		if(value instanceof Double){//解决科学计数法的问题，并保留两位小数
	...
	public static void getExcelField(String targetId, Field field,
		...
		excelEntity.setAlign(excel.align());
	...
	public static HSSFCellStyle getTwoStyle(HSSFWorkbook workbook, boolean isWarp) {
		return getTwoStyle(workbook, isWarp, HSSFCellStyle.ALIGN_CENTER);
	}
	
	public static HSSFCellStyle getTwoStyle(HSSFWorkbook workbook, short align) {
		return getTwoStyle(workbook, true, align);
	}
	
	public static HSSFCellStyle getTwoStyle(HSSFWorkbook workbook, boolean isWarp, short align) {
		HSSFCellStyle style = workbook.createCellStyle();
	...
	public static HSSFCellStyle getOneStyle(HSSFWorkbook workbook, boolean isWarp) {
		return getOneStyle(workbook, isWarp, HSSFCellStyle.ALIGN_CENTER);
	}
	
	public static HSSFCellStyle getOneStyle(HSSFWorkbook workbook, short align) {
		return getOneStyle(workbook, true, align);
	}
	
	public static HSSFCellStyle getOneStyle(HSSFWorkbook workbook, boolean isWarp, short align) {
		HSSFCellStyle style = workbook.createCellStyle();
		style.setBorderLeft((short) 1); // 左边框
		style.setBorderRight((short) 1); // 右边框
		style.setBorderBottom((short) 1);
	...
	private static Map<String, HSSFCellStyle> createStyles(HSSFWorkbook workbook) {
		...
		map.put("oneCenter", getOneStyle(workbook,HSSFCellStyle.ALIGN_CENTER));
		map.put("oneLeft", getOneStyle(workbook,HSSFCellStyle.ALIGN_LEFT));
		map.put("oneRight", getOneStyle(workbook,HSSFCellStyle.ALIGN_RIGHT));
		
		map.put("twoCenter", getTwoStyle(workbook,HSSFCellStyle.ALIGN_CENTER));
		map.put("twoLeft", getTwoStyle(workbook,HSSFCellStyle.ALIGN_LEFT));
		map.put("twoRight", getTwoStyle(workbook,HSSFCellStyle.ALIGN_RIGHT));
		return map;
	}
	...
	private static CellStyle getStyles(Map<String, HSSFCellStyle>  map, boolean needOne, short align) {
		if(needOne && HSSFCellStyle.ALIGN_CENTER == align){
			return map.get("oneCenter");
		}
		if(needOne && HSSFCellStyle.ALIGN_LEFT == align){
			return map.get("oneLeft");
		}
		if(needOne && HSSFCellStyle.ALIGN_RIGHT == align){
			return map.get("oneRight");
		}
		
		if(!needOne && HSSFCellStyle.ALIGN_CENTER == align){
			return map.get("twoCenter");
		}
		if(!needOne && HSSFCellStyle.ALIGN_LEFT == align){
			return map.get("twoLeft");
		}
		if(!needOne && HSSFCellStyle.ALIGN_RIGHT == align){
			return map.get("twoRight");
		}
		
		return map.get("twoCenter");//默认
	}
